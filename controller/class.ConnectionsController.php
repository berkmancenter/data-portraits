...
class ConnectionsController extends DPController {
    ...
    public function go() {
        ...
        if (isset($_POST['username'])) {
            ...
            $friends = "var friends = ";
            $friends .= json_encode($people['friends']);
            $followers = "var followers = ";
            $followers .= json_encode($people['followers']);
            $mutuals = "var mutuals = ";
            $mutuals .= json_encode($people['mutuals']);
            unset($people);
            
            $this->addToView('friends', $friends);
            $this->addToView('followers', $followers);
    ...
    private function crawl($username, $statuses) {
        ...
        $people = UserProcessing::getConnections($connection, $vals, $statuses);
        return $people;
    }
